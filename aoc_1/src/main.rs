...
pub fn collect_codes( line: &str, reg: &Regex ) -> Vec<u32> {
    let x = reg.find(line); // find the next match

    if x.is_none() {
        return vec![];
    }

    let my_match = x.unwrap();

    // convert match to number and add to vector
    let digit: u32 =  match my_match.as_str() {
        "one" | "1" => 1,
        "two" | "2" => 2,
        "three" | "3" => 3,
        "four" | "4" => 4,
    ...
    let mut ans: Vec<u32> = vec![digit];

    // continue function from end of last match
    let magic:usize = if my_match.len() > 1 { 1 } else { 0 };
    ans.extend(collect_codes(&line[my_match.end()-magic..], reg).iter());

    ans
}

pub fn sum_codes( input: &str ) -> u32 {
    let input_lines = input.split("\n");

    let find_re = Regex::new(r"(?x)(one)|(two)|(three)|(four)|(five)|(six)|(seven)|(eight)|(nine)|[0-9]").unwrap();
    let mut accum: u32 = 0;
    for line in input_lines {
        let codes: Vec<u32> = collect_codes( line, &find_re );
        accum += ( codes.first().unwrap() * 10 ) + codes.last().unwrap();
    }
